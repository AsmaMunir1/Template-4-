...
function LatestProducts() {
  ...
  return (
    <div className="w-full bg-white py-20">
      {/* Heading */}
      <h2 className="text-[black] text-4xl text-center font-bold mb-8">Latest Products</h2>
...
<div className="flex justify-center space-x-8 mb-16">
  {["New Arrival", "Best Seller", "Featured", "Special Offers"].map((tab) => (
    <button
      key={tab}
      className="text-[black] text-lg font-medium relative group hover:text-red-600"
    >
      {tab}
      {/* Underline */}
      <span
            ...
            <div className="w-full bg-gray-200 flex justify-center items-center relative overflow-hidden h-[300px] transition-all duration-300 group-hover:bg-white">
              ...
              <div className="absolute top-2 right-2 space-y-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button className="bg-white p-2 rounded-full shadow">
                  <GoHeart  className="w-6 h-6 text-[#3F509E]" />
                </button>
                <button className="bg-white p-2 rounded-full shadow">
                  <PiMagnifyingGlass className="w-6 h-6 text-[#3F509E]" />
                </button>
                <button className="bg-white p-2 rounded-full shadow">
                  <BsCart3   className="w-6 h-6 text-[#3F509E]" />
                </button>
              </div>
            </div>
